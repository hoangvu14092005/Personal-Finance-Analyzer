...
export default function BudgetsClient() {
  ...
  const budgetStats = useMemo(() => {
    ...
    const totalSpent = usages.reduce((sum, usage) => sum + (Number(usage.spent_amount) || 0), 0);
    return {
      totalBudget,
      totalSpent,
      exceeded: usages.filter((usage) => usage.status === "exceeded").length,
      warning: usages.filter((usage) => usage.status === "warning").length,
    };
  }, [budgets, usages]);
      ...
      <header className="space-y-4">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <DisplayLg>Quản lý ngân sách</DisplayLg>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <BudgetStat icon={Target} label="Tổng hạn mức" value={`${formatVnd(String(budgetStats.totalBudget))} VND`} detail={`${budgets.length} danh mục`} tone="neutral" />
          <BudgetStat icon={Wallet} label="Đã sử dụng" value={`${formatVnd(String(budgetStats.totalSpent))} VND`} detail="Theo giao dịch đã ghi sổ" tone="blue" />
          <BudgetStat icon={AlertTriangle} label="Sắp vượt" value={String(budgetStats.warning)} detail="Cần theo dõi" tone="purple" />
          <BudgetStat icon={AlertTriangle} label="Đã vượt" value={String(budgetStats.exceeded)} detail="Ưu tiên xử lý" tone="red" />
        </div>
      </header>
        ...
        <form onSubmit={onSubmit} className="grid gap-4 md:grid-cols-4">
          ...
          <div className="flex items-center gap-2 md:col-span-4">
            <Button type="submit" variant="primary" disabled={submitting}>
              {submitting ? "Đang lưu..." : isEditing ? <><CheckCircle className="h-4 w-4" aria-hidden />Cập nhật</> : <><Plus className="h-4 w-4" aria-hidden />Thêm ngân sách</>}
            </Button>
            {isEditing ? (
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <Badge tone={statusTone(status)}>
                          {status === "safe" ? <Smile className="mr-1 h-3 w-3" aria-hidden /> : <AlertTriangle className="mr-1 h-3 w-3" aria-hidden />}
                          {statusLabel(status)}
                        </Badge>
                      </div>
...
function BudgetStat({ icon: Icon, label, value, detail, tone }: { icon: LucideIcon; label: string; value: string; detail: string; tone: "neutral" | "blue" | "purple" | "red" }) {
  const toneClass = {
