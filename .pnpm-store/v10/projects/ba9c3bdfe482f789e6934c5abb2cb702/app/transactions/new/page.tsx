...
export default function ManualEntryPage() {
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-4">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <DisplayLg>Ghi chép giao dịch thủ công</DisplayLg>
            <p className="mt-0.5 text-sm text-ash">
              Dùng khi bạn muốn ghi nhanh khoản thu hoặc chi mà không cần tải hóa đơn OCR.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link href="/receipts/upload"><Button type="button" variant="secondary" size="sm">Tải hóa đơn lên</Button></Link>
            <Link href="/transactions"><Button type="button" variant="secondary" size="sm">Sổ giao dịch</Button></Link>
          </div>
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          <ManualRule label="Nguồn" value="manual" detail="Không qua OCR" />
          <ManualRule label="Trạng thái" value="confirmed" detail="Tính vào báo cáo sau khi lưu" />
          <ManualRule label="Danh mục" value="user/system" detail="Chọn nhóm chi tiêu phù hợp" />
        </div>
      </header>
      ...
      <Card className="mx-auto max-w-3xl border-hairline-soft bg-surface-card">
        <form onSubmit={onSubmit} className="grid gap-4 md:grid-cols-2">
          <label className="text-body-xs text-ink md:col-span-2">
            Cửa hàng / mô tả giao dịch
            <Input
              type="text"
...
function ManualRule({ label, value, detail }: { label: string; value: string; detail: string }) {
  return (
    <article className="rounded-xl border border-hairline-soft bg-surface-doc p-4">
      ...
      <p className="mt-2 text-heading-sm-mixed text-ink">{value}</p>
      <p className="mt-1 text-caption-sm text-mute">{detail}</p>
    </article>
  );
}
