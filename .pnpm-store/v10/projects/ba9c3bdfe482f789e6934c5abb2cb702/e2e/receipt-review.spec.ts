 ...
 * Phần còn lại (categories list) gọi backend thật; confirm endpoint cũng mock
 * vì receipt id trong test là giả.
 *
 * Test này verify:
...
test.describe("Receipt review form (Phase 3.4)", () => {
  ...
  test("review form pre-fills draft and saves transaction", async ({ page }) => {
    ...
    await page.route(
      `${apiBase}/api/v1/receipts/${fakeReceiptId}/confirm`,
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
          ...
          items: [
            {
              ...
              receipt_upload_id: fakeReceiptId,
              has_invoice: false,
              merchant_name: "Highlands Coffee",
              amount: "75000",
              currency: "VND",
              ...
              source: "ocr",
              status: "confirmed",
              confirmed_at: "2026-01-15T03:00:00Z",
              note: "Cà phê sáng - test E2E",
    ...
    await expect(
      page.getByRole("heading", { name: /Kết quả quét thông tin/i }),
    ).toBeVisible();
    await expect(page.getByText(/85%/)).toBeVisible();

    // Form pre-filled.
    await expect(page.getByLabel("Đơn vị cung cấp")).toHaveValue("Highlands Coffee");
